...
const GetFeedDataComponent = () => {
  ...
  var d=useSelector((state)=>state.getFeedsReducer)
  console.log("d" ,d);
  ...
  useEffect(() => {
    setAllFeeds(feedData);
  }, [feedData]);

  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const result = await dispatch(fetchFeeds({ offset: 0, limit: 10 }));
        const initialFeeds=result.payload;
        if (initialFeeds.length === 0) {
          setHasMore(false);
        } else {
          setAllFeeds(initialFeeds);
...
const fetchMoreData = async () => {
  if (hasMore && !feedLoading) {
    ...
    try {
      const result = await dispatch(fetchFeeds({ offset: newOffset, limit }));
      console.log("New data fetched:", result.payload);
     
      const newFeeds = result.payload;
        ...
        setAllFeeds((prevFeeds) => {
          const uniqueNewFeeds = newFeeds.filter(
            (newFeed) =>
              !prevFeeds.some(
                (existingFeed) =>
                  existingFeed.postid === newFeed.postid ||
                  existingFeed.reels_id === newFeed.reels_id
              )
          );
          const updatedFeeds = [...prevFeeds, ...uniqueNewFeeds];
          return updatedFeeds;
        });
        setOffset(newOffset);
      }
  ...
  return (
    <>
      ...
      <InfiniteScroll
        ...
        loader={feedLoading && <h4>Loading...</h4>}
        endMessage={
