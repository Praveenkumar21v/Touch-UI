  ...
  return (
    <>
      ...
      <div className="modalFlow">
              ...
              <div className="modal-grid">
                ...
                >
                  ...
                  {selectedPost.post_type === "image" ? (
                    ...
                    ref={videoRef}
                      ...
                      className="user-video-media-modal"
                    />
                     {showOverlay && ( 
                       <div className="user-video-overlay" onClick={handleVideoClick}>
                     <FaPlay/>
                     </div>
                    )}
                ...
                <div
                  ...
                  <div
                    className={
                     selectedPost?.comment_count === 0
                        ? "user-modal-profile-right"
                        : "user-modal-profile-ryt"
                    }
                  >
                    <img
                      src={selectedPost?.profile_pic || ""}
                  ...
                  <div className="modal-additional-info">
                    <div className={selectedPost?.comment_count > 0 ?"user-scrollable":"unscrollable"}>
                    <div className={selectedPost?.comment_count > 0 ? "Under-more" : "Under-one"}>
                      {selectedPost?.caption || selectedPost?.description ? (
                        <>
                          <img
                            src={selectedPost?.profile_pic || ""}
                      ...
                      {selectedPost?.caption && (
                        <p className="caption" id={selectedPost.caption.length < 10 ? "caption" : "caption-block-line"}>
                          {selectedPost.caption}
                        </p>
                      )}
                    ...
                    {selectedPost?.comment_count > 0 ? (
                      <div className={selectedPost?.caption || selectedPost?.description ?"user-comments-section":"user-comment-section"}>
                        {Comments &&
                      ...
                      <div className="caption-comment">
                        {selectedPost?.caption || selectedPost?.description ? (
                          <div className="no-comment-provider">
                            {selectedPost?.comment_count === 0 && (
       ...
       {selectedPost?.caption || selectedPost?.description ? (<div
                      className={
                        selectedPost?.comment_count === 0
                          ? "user-post-counters"
                    ...
                    >
                      <div className="user-like-counter">
                        <p>{selectedPost?.like_count || 0}</p>
                      ...
                      <div className="user-comment-counter">
                        {selectedPost?.feed_type === "reel" ? (
                          <p>{reelCommentCount}</p>
                        ) : (
                          <p>{selectedPost?.comment_count || 0}</p>
                        )}
                        ...
                        <div className="post-timer">
                          {selectedPost?.feed_type === "REEL" && (
                            <p className="reel-time" id="reel-time">
                              {calculateTimeDifference(selectedPost?.timestamp)}{" "}
                      ...
                      className={
                        selectedPost?.comment_count === 0
                          ? "user-post-counter"
                          : "user-post-counter-with-comments"
                      }
                    >
                      <div className="user-like-counter">
                        <p>{selectedPost?.like_count || 0}</p>
            ...
            {activeCommentBox ===
              ...
              <div className="highlight-background">
                <textarea
                className={selectedPost?.comment_count > 0 ? "have-comment-box" : "no-comment-box"}
                  id={selectedPost?.caption || selectedPost?.description ? "user-comment-inputs" : "user-comment-input"}
                  placeholder="Write your comment..."
                  value={commentText[selectedPost?.postid || selectedPost?.reels_id] || ""}
                  onChange={(event) => handleInputChange(selectedPost?.postid || selectedPost?.reels_id, event)}
                  onKeyPress={(event) => {
                    if (event.key === "Enter") {
                      handleCommentSubmit();
                    }
                ...
                {showEmojiPicker && (
                  ...
                  >
                    <Picker data={data} theme="dark" navPosition="bottom" skinTonePosition="search"
...
export default ReelUserProfile;